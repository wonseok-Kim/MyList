...
template <typename T> inline void List<T>::push_front(const T& data)
{
    Node* node = new Node;
    node->data = data;
...
template <typename T> inline void List<T>::push_back(const T& data)
{
    Node* node = new Node;
    node->data = data;
...
template <typename T> inline void List<T>::remove(const T& data)
{
    List<T>::iterator iter;
    for (iter = begin(); iter != end();)
    {
...
template <typename T> typename inline List<T>::Node* List<T>::erase(Node* toDelete)
{
    Node* del = toDelete;
...
{
  public:
    const_iterator(const Node* node = nullptr) : m_Node{const_cast<Node*>(node)}
    {
    }

    friend const_iterator List<T>::erase(const_iterator iter);

    const_iterator(iterator iter) : m_Node{iter.m_Node}
    {
    }
